...
def main():
    ...
    print(" MISSINGNESS")
    print(aggregate_series.isnull().sum() * 100 /len(aggregate_series))
    ...
    m5_indices = aggregate_series.loc[aggregate_series['Mortality5Days'] ==1].index
    m5d_comorbidities = aggregate_series.iloc[m5_indices]
    m5d_comorbidities = m5d_comorbidities['comorbidity']

    X = m5d_comorbidities.to_numpy()
    X = X.reshape(-1, 1)
    scaler = MinMaxScaler()
    scaler.fit(X)
    X = scaler.transform(X)
    X = pd.Series(X[:, 0])
    print(" Comorbidity 5 DAY rescaled stats", X.describe())
